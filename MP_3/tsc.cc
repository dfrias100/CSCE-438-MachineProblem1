...
int Client::connectTo()
{
    ...
    // ------------------------------------------------------------
    std::shared_ptr<grpc::Channel> coordChannel = grpc::CreateChannel(hostname + ":" + port, grpc::InsecureChannelCredentials());
    std::unique_ptr<snsCoordinator::SNSCoordinator::Stub> coordStub = snsCoordinator::SNSCoordinator::NewStub(coordChannel);

    grpc::ClientContext grpcCoordContext;

    snsCoordinator::Request coordRequest;
    snsCoordinator::Reply coordReply;

    coordRequest.set_coordinatee(snsCoordinator::CLIENT);
    coordRequest.set_port_num(port);
    coordRequest.set_id(std::stoi(username.substr(1, username.size() - 1)));

    coordStub->Login(&grpcCoordContext, coordRequest, &coordReply);

    StubChannel = grpc::CreateChannel(coordReply.msg(), grpc::InsecureChannelCredentials());
